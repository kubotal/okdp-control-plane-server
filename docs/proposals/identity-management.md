...
> Issue: OKDP/okdp-control-plane-server#25 · UI counterpart: OKDP/okdp-control-plane-ui#41
...
{
  "identity": { "provider": "external", "userManagement": false },
  "oidcProvisioning": { "provider": "keycloak" }
}
```

The UI shows/hides the Identity section based on `identity.userManagement`.
...
- `internal/repository/context_repository.go` — `GetIdentityProvider`,
  `GetIdentityProvisioningProvider`, `GetKeycloakProvisioningConfig`;
  `GetKubauthNamespace` reads `identity.kubauth.namespace` (legacy `kubauth.namespace`
  still honored).
...
- ✅ Context contract getters (`identity.provider`, `identity.provisioning.*`)
...
- ⬜ UI: consume `/api/capabilities` to hide the Identity section (ui#41)
